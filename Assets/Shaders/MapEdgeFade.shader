Shader "UI/MapEdgeFade"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // Sample the map texture, tinted by the RawImage colour field
                fixed4 col = tex2D(_MainTex, IN.texcoord) * IN.color;

                // Centre UVs, correct for aspect ratio
                float2 uv = IN.texcoord - 0.5;
                uv.x *= _AspectRatio;

                // Superellipse distance — n=2 circle, n=20 near-square with smooth corners
                float n    = _Roundness;
                float dist = pow(pow(abs(uv.x), n) + pow(abs(uv.y), n), 1.0 / n) * 2.0;

                // Fade alpha toward edges; centre is fully opaque, edges transparent
